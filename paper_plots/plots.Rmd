...
subset_sankey = table_s1 %>% 
  select(portal, donor_sex, organ_name, cell_type_annotation_tool, donor_race, donor_bmi) %>% 
  replace_na(list(donor_sex = "unknown")) %>% 
  replace_na(list(donor_race = "unknown")) %>% 
  # mutate(donor_bmi = as.character(donor_bmi)) %>% 
  # replace_na(list(donor_bmi = "unknown")) %>%
...
unique_name=list()
unique_name = unlist(append(unique_name, c(p, d, r, o, c)))
...
prep_links = as.data.frame(bind_rows(s_o, r_o, d_o, c_o))
prep_links 
...
p <- sankeyNetwork(Links = prep_links, Nodes = nodes, Source = "source",
                   ...
                   units = "occurrences", fontSize = 20, nodeWidth = 40, width=940)
