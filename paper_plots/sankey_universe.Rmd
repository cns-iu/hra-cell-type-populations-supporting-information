...
### Load CxG developmental stage (not in graph yet)
```{r}
...
### Replace NAs in donor_developmental_stage  with values from cxg_age
```{r} 
...
### Assign developmetal_stage to bins
```{r}

# sankey_universe_with_bins <- sankey_universe_with_bins %>% 
#   mutate(
#     AgeCategory = case_when(
#       AgeDescription %in% c("young", "young adult") ~ "Young",
#       AgeDescription %in% c("middle-aged", "adult") ~ "Adult",
#       AgeDescription %in% c("senior", "old", "elderly") ~ "Senior",
#       TRUE ~ "Unknown"
#     )
#   )
...
subset_sankey = sankey_universe_with_bins %>% 
  select(portal, donor_sex, organ_name, dataset_id, cell_type_annotation_tool, donor_race, donor_bmi_binned, donor_age_binned, is_rui_registered, is_atlas_dataset) %>% 
...
```{r}
  ...
  # subset_sankey$cell_type_annotation_tool[subset_sankey$cell_type_annotation_tool == "azimuth"] <- "sc_transcriptomics with cell summary"
  # subset_sankey$cell_type_annotation_tool[subset_sankey$cell_type_annotation_tool == "celltypist"] <- "sc_transcriptomics with cell summary"
  # subset_sankey$cell_type_annotation_tool[subset_sankey$cell_type_annotation_tool == "popv"] <- "sc-transcriptomics with cell summary"# 
