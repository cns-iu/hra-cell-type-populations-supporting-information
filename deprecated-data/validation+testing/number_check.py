...
def main():
    ...
    # get unique AS
    for ana in as_cell_summaries['@graph']:
        unique_as.add(ana['cell_source'])
    ...
    # get number of datasets with rui location and modality
    for donor in enriched_rui_locations['@graph']:
        for sample in donor['samples']:
            for dataset in sample['datasets']:
                if "summaries" in dataset:
                    for sum in dataset['summaries']:
                        unique_datasets_with_rui_locations[sum['modality']].add(dataset['@id'])
                        # print(sum['modality'])
    ...
    print(f'''
          ...
          Unique datasets with RUI location (bulk): {len(unique_datasets_with_rui_locations['bulk'])}
          Unique datasets with RUI location (spatial): {len(unique_datasets_with_rui_locations['spatial'])}
          Unique AS summaries: {len(unique_as)}
          ''')
    ...
    with open('corridors_with_their_as.csv', 'w', newline='') as f:
        ...
        wr.writerow(['corridor', 'number_of_unique_as', 'AS1',
