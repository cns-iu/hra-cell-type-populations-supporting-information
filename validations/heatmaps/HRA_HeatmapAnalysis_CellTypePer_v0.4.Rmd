...
write.csv(metadata,"../heatmaps/ct_pop_organ_dataset_metadata_pivot.csv",
          row.names = F)
metadata_pivot <- 
  metadata %>%
  ddply(.(organ), summarise,
        tool_count = length(unique(tool)),
        as_count = length(unique(as)),
        min_ds = min(ds_count),
        max_ds = max(ds_count)) %>%
  arrange(desc(tool_count), desc(as_count),organ)

write.csv(metadata_pivot,"../heatmaps/ct_pop_organ_dataset_metadata_pivot2.csv",
          row.names = F)
```
