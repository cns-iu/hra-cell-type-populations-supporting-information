...
# Testing Branch 
api_url <- c("https://raw.githubusercontent.com/x-atlas-consortia/hra-pop/refs/heads/v0.12.0/output-data/")
output_data_dir <- c("v0.12.0")
...
data$organ <- 
  ...
  str_replace("left ", "") %>%
  str_replace("right ", "") %>%
  str_replace("Set of lactiferous glands in ", "") %>%
  str_replace(" of body", "") %>% 
  str_replace("respiratory system","lung") %>%
  str_replace("male reproductive system","prostate") %>%
  str_to_title()
...
# 1 Breasts
breast <- data[data$organ==levels(data$organ)[1],]
# 2 Heart
heart <- data[data$organ==levels(data$organ)[2],]
# 3 Kidney
kidney <- data[data$organ==levels(data$organ)[3],]
# 4 Large Intestine
largeIntestine <- data[data$organ==levels(data$organ)[4],]
# 5 Lungs
lung <- data[data$organ==levels(data$organ)[5],]
# 6 Pancreas 
pancreas <- data[data$organ==levels(data$organ)[6],]
# 7 Prostate
...
# 11 Thymus
thymus <- data[data$organ==levels(data$organ)[11],]
# 12 Ureter 
ureter <- data[data$organ==levels(data$organ)[12],]
# 13 Urinary bladder
bladder <- data[data$organ==levels(data$organ)[13],]
...
# Pancreas
tmp4 <- 
  join(pancreas[pancreas$tool=="Azimuth",c(1:11)],
       azimuth[azimuth$organ_level=="Pancreas_L1" &
               azimuth$cell_match=="skos:exactMatch" | 
               azimuth$cell_match=="skos:narrowMatch", c(3:6)], 
       by="cell_id", type="left")
# # Ureter
# tmp5 <-
#   join(ureter[ureter$tool=="Azimuth",c(1:11)],
#        azimuth[azimuth$organ_level=="Kidney_L3" &
#                azimuth$cell_match=="skos:exactMatch" | 
#                azimuth$cell_match=="skos:narrowMatch", c(3:6)], 
#        by="cell_id", type="left")
# Clean names
 names(tmp1) <- names(tmp2) <- names(tmp3) <- names(tmp4) <- #tmp5 <-
...
# Pancreas
tmp7 <- 
  join(pancreas[pancreas$tool=="CellTypist",c(1:11)],
       azimuth[azimuth$organ_level=="Pancreas_L1" &
               azimuth$cell_match=="skos:exactMatch" | 
               azimuth$cell_match=="skos:narrowMatch", c(3:6)], 
       by="cell_id", type="left")
...
# Clean names
 names(tmp1) <- names(tmp2) <- names(tmp3) <- names(tmp4) <- #names(tmp5) <- 
   names(tmp6) <- names(tmp7) <-
   c("sex","tool","modality","organ","organId","as","as_id",
     "cell_id","cell_label","cell_count", "percentage",
     "annotation_label","annotation_label_id","cell_label2")
 
# Combine Azimuth temp files
data_celltypist <- rbind(tmp1,tmp2,tmp3,tmp4,#tmp5,
                         tmp6,tmp7)
...
# Pancreas
tmp7 <- 
  join(pancreas[pancreas$tool=="popV",c(1:11)],
...
# Breasts
tmp8 <- 
  join(breast[breast$tool=="popV",c(1:11)],
       popv[popv$organ_level=="mammary gland" &
            popv$cell_match=="skos:exactMatch" | 
            popv$cell_match=="skos:narrowMatch", c(3:6)], 
       by="cell_id", type="left")
# Thymus
tmp9 <- 
  join(thymus[thymus$tool=="popV",c(1:11)],
       popv[popv$organ_level=="mammary gland" &
            popv$cell_match=="skos:exactMatch" | 
            popv$cell_match=="skos:narrowMatch", c(3:6)], 
       by="cell_id", type="left")
...
# Ureter
tmp13 <- 
  join(ureter[ureter$tool=="popV",c(1:11)],
       popv[popv$organ_level=="mammary gland" &
            popv$cell_match=="skos:exactMatch" | 
            popv$cell_match=="skos:narrowMatch", c(3:6)], 
       by="cell_id", type="left")
...
# Clean names
names(tmp1) <- names(tmp2) <- names(tmp4) <- names(tmp5) <- 
  names(tmp6) <- names(tmp7) <- names(tmp8) <- names(tmp9) <- 
  names(tmp10) <- names(tmp11) <- names(tmp12) <- names(tmp13) <-
  #names(tmp3) <-
  c("sex","tool","modality","organ","organId","as","as_id",
    "cell_id","cell_label","cell_count", "percentage",
    "annotation_label","annotation_label_id","cell_label2")
...
# Combine Azimuth temp files
data_popv <- rbind(tmp1,tmp2,tmp4,tmp5,tmp6,tmp7,
...
# Lung
tmp4 <- 
  lung[lung$tool=="SC Proteomics",c(1:11)] %>%
  mutate(annotation_label=NA,
         annotation_label_ids=NA,
         cell_label2=NA)
# Spleen
tmp5 <- 
  spleen[spleen$tool=="SC Proteomics",c(1:11)] %>%
  mutate(annotation_label=NA,
         annotation_label_ids=NA,
         cell_label2=NA)
...
# Combine Azimuth temp files
data_scproteomics <- rbind(tmp1, tmp2, tmp3, tmp4, tmp5)
rm(tmp1, tmp2, tmp3, tmp4, tmp5)
...
rm(heart, lung, kidney, largeIntestine, smallIntestine, skin, bladder, #liver,
   breast, prostate, spleen, pancreas, thymus, ureter)
