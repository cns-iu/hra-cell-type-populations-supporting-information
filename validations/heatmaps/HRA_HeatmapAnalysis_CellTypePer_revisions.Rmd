...
```{r all_tools_heatmap, fig.height=12, fig.width=48}
# Create pivot
...
```{r azimuth_heatmap, fig.height=12, fig.width=24}
# Create pivot
data_ha <- data %>% 
              filter(tool == "azimuth") %>%
...
```{r celltypist_heatmap, fig.height=12, fig.width=36}
# Create pivot
data_hc <- data %>% 
...
```{r popV_heatmap, fig.height=12, fig.width=24}
# Create pivot
data_hp <- data %>% 
              filter(tool == "popv") %>%
              select(organ, as, cell_name, percentage) %>%
