...
for(i in 1:length(df_list)){
    ...
    organs <- 
      df_list[[i]] %>% 
      select(tool, organId, organ, cell_count) %>%
      ddply(.(tool, organId, organ), summarise,
            records = length(cell_count)) %>%
      mutate(id_lab = paste0(organId," ",organ),
             type = "Organ") %>% 
      dplyr::rename(label = organ) %>%
      select(tool, id_lab, label, type, records)
    
    anatomical_structures <-
      df_list[[i]] %>% 
      select(tool, as_id, as, cell_count) %>%
      ddply(.(tool, as_id, as), summarise,
            records = length(cell_count)) %>%
    ...
    cell_types <- 
      df_list[[i]] %>% 
      select(tool, cell_id, cell_label, cell_count) %>%
    ...
    nodes <- 
      ...
      select(id, id_lab, label, tool, type, records)

    ## Edge list creation  
    organ_as <-
      df_list[[i]] %>% 
      mutate(source_id_lab = paste0(organId," ",organ),
             target_id_lab = paste0(as_id," ",as)) %>%
      ddply(.(source_id_lab,target_id_lab), summarise,
            weight = 1,
            weight_recs = length(tool),
            weight_tfidf = sum(tfidf)) 
    ...
    as_celltypes <- 
      ...
      mutate(source_id_lab = paste0(as_id," ",as),
             ...
             weight = 1) %>%
      select(source_id_lab, target_id_lab, weight, weight_recs, weight_tfidf)
    
    # Combine edge lists
    edges <- rbind(organ_as, as_celltypes) %>% 
      distinct()
    rm(organ_as, as_celltypes)
