...
data <- left_join(data,data_set_meta, by="dataset_id") %>%
            select(dataset_id, organ, as_uberon_label, 
                   cell_type, count) %>%
            rename(as = as_uberon_label,
                   id = dataset_id)
...
data_total_cell_count <- data %>% 
                          ddply(.(organ,as), summarise,
                                  total_cell_count = sum(count))
# Organ-AS-CT counts 
data_agg <- data %>% 
              ddply(.(organ,as,cell_type), summarise,
                    datasets = length(unique(id)),
                    cell_type_count = sum(count),
                    mean_count = median(count))

# Combine aggregated data - organ - as - ct counts and percentages.
data <- left_join(data_agg,data_total_cell_count, by=c("organ","as")) %>%
          select(organ,as,cell_type,datasets,mean_count,cell_type_count,total_cell_count) %>%
          mutate(percentage = round(cell_type_count/total_cell_count,5))
...
for(i in 1:length(df_list)){
    ...
    cell_types <- 
      df_list[[i]] %>% 
      select(organ, cell_type, datasets, cell_type_count) %>%
      ddply(.(cell_type), summarise,
            label = head(cell_type)[1],
            records = sum(datasets),
            cell_count = sum(cell_type_count),
            partOf = length(unique(organ))) %>%
      mutate(id_lab = cell_type,
             type = "Cell Type",
