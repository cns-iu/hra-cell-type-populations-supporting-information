...
kid <- df[df$organ==levels(df$organ)[2],] %>%
  ...
  filter(mean_cell_pt >= .10)
kid$as <- factor(kid2$as)
kid$cell_lab <- factor(kid2$cell_lab,
                        levels=c("CL:1000718: Inner Medullary Collecting Duct",
...
kid %>% arrange(sex,tool,as,mean_cell_pt) %>%
        ggplot(aes(x=as,y=mean_cell_pt)) +
          ...
          labs(x="Anatomical Structure", y="Percentage (%)",
               title="Kidney Anatomical Structures - Cell Types greater than 10% share, by Donor Sex and Analysis Tool") +
