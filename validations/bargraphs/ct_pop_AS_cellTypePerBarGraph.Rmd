...
piv10 <- df2 %>% 
  select(organ, as, cell_id, cell_count, percentage) %>%
  distinct() %>%
  ddply(.(organ, as, cell_id), summarise,
...
piv20 <- piv10 %>% 
  select(organ, as, cell_id, measure_count, 
         mean_cell_count, mean_cell_percentage) %>% 
  ddply(.(organ, cell_id), summarise,
        as_count = length(as),
...
# Organ Count
piv30 <- piv20 %>% 
  select(organ, cell_id, as_count, measure_count, 
         mean_cell_count, mean_cell_percentage) %>% 
  ddply(.(cell_id), summarise,
...
```{r bar_heart, fig.height=14, fig.width=12}
# Subset df2 to relevant organ
barGraph <- df2[df2$organ==levels(df$organ)[1],]

# Filter cell types to remove multiorgan cell types.
